...
ALIGN_WITH_STARTING_ZONE_FOR_REAL = False
...
class GrabPuckIT(IntegrationContext):
    ...
    def route_station_command(self):
        ...
        # TODO Add support for rotation command
        if topic == Topic.START_CYCLE:
            ...
            print("Grabbing puck")
            self.grab_puck(command.get_payload())
    ...
    def align_with_puck(self, puck_color: Color):
        if ALIGN_WITH_PUCK_FOR_REAL:
            self._puck_handler.align_with_puck(puck_color)
        else:
