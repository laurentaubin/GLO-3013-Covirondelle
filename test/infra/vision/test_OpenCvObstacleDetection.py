...
class TestOpenCvObstacleDetector(TestCase):
    ...
    def setUp(self) -> None:
        self.obstacle_detector = OpenCvObstacleDetector(self.ARUCO_DICTIONARY, self.OBSTACLE_ARUCO_MARKER_ID)

    def test_givenAnImageWithTwoObstacles_whenDetectObstacle_thenReturnCorrectObstaclePosition(self):
        an_image = cv2.imread(self.AN_IMAGE)
        expected_first_obstacle_position, expected_second_obstacle_position = [Position(1156, 805), Position(803, 252)]
